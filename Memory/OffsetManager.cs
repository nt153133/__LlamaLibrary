...
namespace LlamaLibrary.Memory
{
    public static class OffsetManager
    {
        ...
        public static async Task<bool> Init()
        {
            ...
            try
            {
                if (await InitLock.WaitAsync(new TimeSpan(0, 0, 30)))
                {
                    try
                    {
                        stopwatch.Restart();
                        ...
                        initStarted = true;
                        if (await InitLock1.WaitAsync(new TimeSpan(0, 0, 30)))
                        {
                            try
                            {
        ...
        private static async Task SearchAndSetLL()
        {
            ...
            await SetOffsetObjects(llTypes);
        }
        ...
        public static async Task SetOffsetObjects(IEnumerable<Type> q1)
        {
